...
class Visualizer:
    ...
    def __init__(self, model, intensity_range, regularization, input_shape,
        ...
        pattern = np.zeros(input_shape) # (55, 47, 3)
        mask = np.expand_dims(mask, axis=2) # (55, 47, 1)

        mask_tanh = np.zeros_like(mask)
        pattern_tanh = np.zeros_like(pattern)

        # prepare mask related tensors
        self.mask_tanh_tensor = K.variable(mask_tanh) # (55, 47, 1)
        ...
        mask_tensor_unexpand = K.repeat_elements(
            ...
            rep=self.img_color,
            axis=2) # (55, 47, 3)
        self.mask_tensor = K.expand_dims(mask_tensor_unexpand, axis=0) # (1, 55, 47, 3)
